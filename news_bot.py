...
SOURCES = {
    ...
    "조선에듀": "http://edu.chosun.com/site/data/html_dir/list.html"
}
# ----------------
...
def get_news():
    ...
    # 1. 베리타스알파
    try:
        v_url = "http://www.veritas-a.com/news/articleList.html?sc_section_code=S1N1"
    ...
    # 2. 에듀진
    try:
        e_url = "http://www.edujin.co.kr/news/articleList.html?sc_section_code=S1N2"
        res = requests.get(e_url, headers=headers)
        soup = BeautifulSoup(res.text, 'html.parser')
        titles = [a for a in soup.select('a[href*="articleView"]') if a.get_text(strip=True) and not is_ad(a.get_text(strip=True))]
        for t in titles[:3]:
            href = t['href']
    ...
    # 3. 에듀동아
    try:
        res = requests.get("http://edu.donga.com/?p=1&ps=news", headers=headers)
        soup = BeautifulSoup(res.text, 'html.parser')
        titles = [a for a in soup.select('a[href*="articleView"]') if a.get_text(strip=True) and not is_ad(a.get_text(strip=True))]
        seen = set()
        count = 0
    ...
    # 4. 조선에듀
    try:
        res = requests.get("https://edu.chosun.com", headers=headers)
        soup = BeautifulSoup(res.text, 'html.parser')
